...
export async function signup(req, res) {
  ...
  if (username.includes("@", "$")) {
    return res
      .status(400)
      .json({ detail: "Username shouldn't contain '@' or special characters" });
  }
  ...
  try {
    ...
    res.status(201).send({ detail: "Account created successfully", user: modifiedUser });
  } catch (e) {
...
export async function refreshAccessToken(req, res) {
  ...
  if (!refreshToken) {
    return res
      .status(400)
      .send({ detail: "Refresh token not available in request body." });
  }
